...
def naiveSoftmaxLossAndGradient(
    ...
    # g_vc
    loss = - np.log(np.exp(outsideVectors[outsideWordIdx].dot(centerWordVec)) / np.sum(np.exp(outsideVectors.dot(centerWordVec)),axis=0))
    denominator = np.sum(np.exp(outsideVectors.dot(centerWordVec)))
    gradCenterVec = - outsideVectors[outsideWordIdx] + outsideVectors.T.dot(np.exp(outsideVectors.dot(centerWordVec))/denominator)
    
    P_w_c = np.exp(outsideVectors.dot(centerWordVec)) / denominator
    P_w_c[outsideWordIdx] -= 1
    # Becareful with the direction of the output of function tile,
    gradOutsideVecs = (np.tile(centerWordVec, outsideVectors.shape[0]).reshape(outsideVectors.shape[0],centerWordVec.shape[0]).T.dot(np.diag(P_w_c))).T
...
def negSamplingLossAndGradient(
    ...
    counts = {i:negSampleWordIndices.count(i) for i in set(negSampleWordIndices)}
    indices = [outsideWordIdx] + negSampleWordIndices
    KMatrix = outsideVectors[negSampleWordIndices]
    loss = - np.log(sigmoid(outsideVectors[outsideWordIdx].dot(centerWordVec) )) - np.sum(np.log(sigmoid( - KMatrix.dot(centerWordVec))))
    mindVar = 1-sigmoid(-KMatrix.dot(centerWordVec))
    tmp = mindVar * (KMatrix).T
    gradCenterVec = np.sum(tmp,axis=1) +  (sigmoid(outsideVectors[outsideWordIdx].dot(centerWordVec))-1) * outsideVectors[outsideWordIdx]
    gradOutsideVecs = np.zeros((len(outsideVectors),len(centerWordVec),))
    OutsideList = list(set(negSampleWordIndices))
    for Idx in OutsideList:
        gradOutsideVecs[Idx,:] = counts[Idx] *(1-sigmoid(-outsideVectors[Idx].dot(centerWordVec))) * centerWordVec
